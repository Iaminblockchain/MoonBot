    ...
    try {
        ...
        const accounts = await connection.getProgramAccounts(
            MAINNET_PROGRAM_ID.AmmV4, // Program ID for the Raydium AMM V4
            {
                ...
                filters: [
                    // Filters to narrow down the search for relevant accounts
                    { dataSize: LIQUIDITY_STATE_LAYOUT_V4.span }, // Filters by the size of the liquidity state layout
                    {
                        memcmp: {
                            // Memory comparison filter to match the base mint
                    ...
                    {
                        memcmp: {
                            // Memory comparison filter to match the quote mint
